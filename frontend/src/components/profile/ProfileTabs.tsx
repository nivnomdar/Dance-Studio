    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-[#EC4899]/10">
        <div className="p-3 sm:p-4">
          <div className="flex flex-wrap gap-1 sm:gap-2 justify-center sm:justify-start" role="tablist">
            {tabs.map((tab) => {
              const Icon = tab.icon;
              ...
              return (
                ...
                >
                  ...
                  {tab.count !== null && (
                    <span className={`px-1 sm:px-1.5 py-0.5 rounded-full text-xs font-bold ${isActive ? 'bg-white/20' : 'bg-[#EC4899] text-white'}`}>
                      {tab.count}
                    </span>
                  )}
                </button>
