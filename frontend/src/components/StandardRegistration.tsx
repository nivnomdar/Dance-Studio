...
// Minimal safe fallback to prevent runtime errors and keep UI consistent
// You can extend this later with full standard registration flow
const StandardRegistration: React.FC<StandardRegistrationProps> = ({ classData }) => {
  ...
  return (
    <div className="bg-white rounded-2xl p-8 shadow-lg h-fit">
      <h2 className="text-2xl font-bold text-[#4B2E83] mb-4">הרשמה לשיעור</h2>
      ...
      {!user ? (
        <div className="text-[#4B2E83]/80 text-sm">
          אנא התחברי כדי להירשם לשיעור זה. לאחר ההתחברות תופיע אפשרות ההרשמה.
        </div>
      ) : (
        <div className="text-[#4B2E83]/80 text-sm">
          ההרשמה לשיעור זה תתאפשר בקרוב דרך האתר. לביצוע הרשמה ידנית כעת, ניתן לפנות ב־Whatsapp.
        </div>
      )}
