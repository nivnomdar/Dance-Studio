...
const ClassCard: React.FC<ClassCardProps> = ({ classItem, usedTrialClassIds }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative h-44 sm:h-56 block">
        <img
      ...
      {/* Content */}
      <div className="p-4 sm:p-6 flex flex-col h-full pt-4 sm:pt-6">
        <h3 id={`class-title-${classItem.id}`} className={`text-base sm:text-xl font-bold ${colorScheme.textColor} mb-2 sm:mb-3 font-agrandir-grand line-clamp-2`}>
          {classItem.name}
        </h3>

        <div className="h-18 sm:h-20 mb-3 sm:mb-4">
          <p id={`class-description-${classItem.id}`} className="text-[#2B2B2B] font-agrandir-regular leading-relaxed text-xs sm:text-sm line-clamp-3">
            {classItem.description}
          </p>
        </div>

        {/* Class Details */}
        <div className="space-y-2 mb-5 sm:mb-6 h-12 sm:h-14">
          {classItem.duration && (
        ...
        <div className="lg:mt-auto">
          {hasUsedTrial ? (
            <div className="inline-flex items-center justify-center w-full bg-gray-500 text-white px-3 lg:px-4 py-1.5 sm:py-2 rounded-xl font-medium text-xs sm:text-sm cursor-not-allowed opacity-90" role="status" aria-label="שיעור ניסיון שכבר נוצל">
              נוצל
