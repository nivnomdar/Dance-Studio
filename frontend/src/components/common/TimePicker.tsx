...
export default function TimePicker({
  ...
  return (
    ...
    >
      <div id="time-picker-title" className="sr-only">בחירת שעה</div>
      <div id="time-picker-description" className="sr-only">בחרי שעה מהאפשרויות הזמינות</div>
        ...
        <>
          ...
          <div className="mb-3 p-2 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200">
            <button
              ...
              aria-label={`השעה הנוכחית: ${new Date().toLocaleTimeString('he-IL', { hour: '2-digit', minute: '2-digit' })}`}
            >
          ...
          <div className="mb-3 p-2 bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg border border-gray-200">
            ...
            <div className="grid grid-cols-3 gap-1" role="group" aria-label="שעות נפוצות">
          ...
          <div className="mb-3 p-2 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
            <label htmlFor="manual-time-input" className="text-xs font-medium text-blue-700 mb-1 block">
              הזנת שעה ידנית:
            </label>
            <input
        ...
        <>
          ...
          {availableTimes.length > 0 ? (
            <div className="mb-3 p-2 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
              ...
              <div className="flex flex-wrap gap-1" role="group" aria-label="שעות זמינות">
                ...
                {availableTimes.length > 6 && (
                  <span className="text-xs text-blue-600 font-medium" aria-label={`ועוד ${availableTimes.length - 6} שעות נוספות`}>
                    +{availableTimes.length - 6} נוספות
                  </span>
                )}
              </div>
            </div>
          ) : (
            <div className="mb-3 p-2 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-lg border border-yellow-200" role="alert" aria-live="polite">
