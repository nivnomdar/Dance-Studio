...
export default function UserSearch({
  ...
  return (
    <div className="relative">
      ...
      <div className="relative">
        ...
        {isOpen && filteredUsers.length > 0 && (
          <div
            ...
            role="listbox"
            aria-label="תוצאות חיפוש משתמשים"
          >
            {filteredUsers.map((user, index) => (
              <button
                ...
                aria-label={`${user.first_name} ${user.last_name}, ${user.email}${user.phone_number ? `, ${user.phone_number}` : ''}`}
              >
                <div className="flex items-center gap-4 w-full">
                  {/* User Avatar */}
                  <div className="w-10 h-10 bg-gradient-to-br from-[#EC4899] to-[#4B2E83] rounded-full flex items-center justify-center flex-shrink-0 shadow-sm" aria-hidden="true">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 text-xs text-gray-600">
                        <FaEnvelope className="w-3 h-3 flex-shrink-0 text-[#EC4899]" aria-hidden="true" />
                        <span className="truncate">{user.email}</span>
                      </div>
                      {user.phone_number && (
                        <div className="flex items-center gap-2 text-xs text-gray-600">
                          <FaPhone className="w-3 h-3 flex-shrink-0 text-[#4B2E83]" aria-hidden="true" />
        ...
        {isOpen && searchTerm.trim() && filteredUsers.length === 0 && !isSearching && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-gray-100 rounded-xl shadow-xl z-20 p-6" role="status" aria-live="polite">
      ...
      {selectedUser && (
        <div className="mt-4 p-4 bg-gradient-to-r from-[#EC4899]/5 to-[#4B2E83]/5 border-2 border-[#EC4899]/20 rounded-xl shadow-sm" role="status" aria-live="polite">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <div className="w-12 h-12 bg-gradient-to-r from-[#EC4899] to-[#4B2E83] rounded-full flex items-center justify-center flex-shrink-0 shadow-sm" aria-hidden="true">
      ...
      {error && (
        <div className="mt-3 p-4 bg-red-50 border-2 border-red-200 rounded-xl" role="alert" id="user-search-error">
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 bg-red-500 rounded-full flex items-center justify-center flex-shrink-0" aria-hidden="true">
